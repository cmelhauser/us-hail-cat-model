...
def test_stage04b_uses_shared_250mm_qa_cap():
    s = load_stage("04b_fill_gridrad_gap.py")
    repaired, n_bad = s.sanitize_hail_values(np.array([[0.0, 249.0, 251.0]], dtype=np.float32))
    assert s.QA_MAX_HAIL_MM == 250.0
    assert n_bad == 1
    assert repaired.tolist() == [[0.0, 249.0, 0.0]]
