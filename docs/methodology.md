...
GridRad fills the gap between MYRORSS and operational MRMS. Stage 04b downloads GridRad / GridRad-Severe inputs from NCAR RDA/GDEX. Stage 04c computes Severe Hail Index (SHI) from three-dimensional **radar reflectivity in dBZ** and ERA5 isotherm fields, then converts SHI to MESH75. GridRad-Severe is preferred where available because higher temporal sampling better resolves short-lived hail cores. When **04c** chains downloads (`--with-04b-download`), it uses a **severe-first** policy: severe files are fetched when the catalog lists them; hourly GridRad is downloaded only when severe is unavailable or does not cover the full convective window, and at processing time hourly timesteps may fill gaps in partial severe coverage. Hourly fallback order is **d841000** (GridRad V3.1, through 2017, all months) then **d841001** (GridRad V4.2 warm-season hourly, Apr–Aug 2008–2021) for convective days after 2017 when the V3.1 catalog is empty.
...
2. **SPC-derived range debias (opt-in research)** — disabled by default so SPC remains validation-only (AGENTS rule #3). Enable with Stage 05 / `run_pipeline.py --allow-spc-derived-adjustments` (ignored with `--skip-ml`). `scripts/diagnostics/radar_artifact_diagnostic.py` fits per-era multiplicative factors from Stage 06 SPC–MESH pairs binned by distance to the nearest CONUS WSR-88D site. Factors are normalized to **1.0 at 125 km** and clipped to **[0.45, 1.15]**. Applied only when allowed and `data/analysis/calibration/range_debias.npz` exists (`--no-range-debias` to disable even when allowed).
...
4. **Optional geometry-aware hail-likelihood classifier (v2.3.0+, opt-in research)** — same `--allow-spc-derived-adjustments` gate; **GridRad days only**. When `artifact_classifier.pkl` exists, Stage **05** applies a `GradientBoostingClassifier` trained on Stage **06** SPC weak labels (severe report at cell = positive; high-MESH no-report = negative). Features (`scripts/_artifact_features.py`): MESH, log MESH, local median ratio, range/azimuth to nearest WSR-88D, month, era flags. Predicted **hail likelihood** multiplicatively down-weights active cells (not a hard 0.65 zeroing of “artifact probability”). Train with `scripts/train_artifact_classifier.py` (GridRad-only by default; year-grouped holdout) or `--retrain-models`. Missing artifact or `--skip-ml` selects the complete deterministic rules-only path (AGENTS rule #2).
...
**2026-07-06 (four-pass, neighbor-only radial):** `remove_radial_range_rings()` added;
Stage 05 rebuilt (~110 min). Post-filter diagnostic on 9,797 days:

| Metric | Three-pass (2026-07-05) | Four-pass (2026-07-06) |
|--------|------------------------:|-----------------------:|
| GridRad mean speckle fraction | 6.1% | **1.8%** |
| GridRad P95 speckle fraction | 33% | **9.1%** |
| GridRad / MYRORSS mid-range ratio (50–125 km) | ~1.7× | ~1.7× |
| Residual rings in diff map | visible | **still visible** (wide mid-range plateaus + era bias) |

**2026-07-06 (inner-range refinement):** radial pass now compares outer bins (≥50 km) to
an inner-range baseline (≤75 km) as well as ±1/±2 neighbors; thresholds **1.12×** /
**1.18×** (was 1.20× / 1.25×). Stage 05–07 rebuild via `logs/pipeline_05_07.run.log`.
