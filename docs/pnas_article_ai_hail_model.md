...
**Status:** Complete draft — v2.2.2 production hazard results; five-pass GridRad artifact filter with site-specific remediation (2026-07-07)  
...
**Range-dependent debias and GridRad artifact filter (2026-07).** Stochastic return-period maps initially showed NEXRAD range rings and spokes, traced to GridRad-era isolated speckle and range-dependent over-estimation relative to SPC collocated pairs. We added `scripts/diagnostics/radar_artifact_diagnostic.py`, which bins validation pairs by distance to the nearest CONUS WSR-88D site (~140 radars) and fits per-era multiplicative debias factors (normalized at **125 km**, clipped **[0.45, 1.15]**). Stage 05 applies `range_debias.npz` when present. GridRad days pass a **five-pass artifact filter**: isolated speckle (&gt; **2.5×** local 3×3 median), **radial range ring** (per-site 10 km bins vs inner-range ≤75 km baseline and neighbors; **1.12×** / **1.18×** thresholds), azimuthal annulus, quiet-background filament, and **site-specific remediation** on nine WSR-88D radars flagged by diff-map QA (**KLRX** northern Nevada, **KEMX** southern Arizona, **KBLX** south-central Montana, **KGRR** west-central Michigan, **KGWX** northwest Alabama, **KTLX** east-central Oklahoma, **KILN** southwest Ohio, **KHPX** southwest Kentucky, **KDOX** central Delaware). The fifth pass re-applies passes 1–4 with stricter thresholds under each site's nearest-radar domain and adds a polar (range × **15°** azimuth) spoke test (**1.5×** sector median), following Murillo et al. (2021) per-radar volume exclusion and WSR-88D range–azimuth median-filter practice. On the **9,797-day** archive, GridRad mean speckle fell from **9.7%** (pre-debias) to **6.1%** (three-pass) to **2.0%** (post–inner-range rebuild; P95 **11.1%**). Residual GridRad−MYRORSS mean-annual-max offsets (~**1.7×** at 50–125 km) reflect era sampling and mid-range calibration bias as well as per-day artifacts. Stages **05–07** were rebuilt after each filter revision; full hazard Stages **08–14** pending artifact QA.

Optional ML calibration and probabilistic filtering remain available; `--skip-ml` forces deterministic fallbacks.
...
| 9 | Stochastic RP radar rings/spokes | GridRad speckle 9.7%; SPC/MESH ~0.81 at 112 km; visible NEXRAD geometry in RP maps | `radar_artifact_diagnostic.py`; `range_debias.npz`; four-pass GridRad filter in Stage 05 | Speckle 9.7%→6.1%→**1.8%**; inner-range radial pass (2026-07-06) | Residual GridRad−MYRORSS climatology offset (~1.7× mid-range); diff-map rings in NE/Plains |
...
After the first full v2.2.1 hazard run, **100-year** and **10,000-year** stochastic return-period maps showed concentric NEXRAD rings and radial spokes over the central and eastern CONUS—patterns inconsistent with physical hail climatology. AI-assisted review linked the artifacts to GridRad-era speckle (**9.7%** of active cells vs **3.9%** for MYRORSS) and range-dependent SPC/MESH bias (median report/MESH ratio **~0.81** at 112 km from radar). The team implemented a diagnostic that bins validation pairs by nearest-radar distance, fits per-era debias factors, and applies a four-pass GridRad artifact filter in Stage 05 (speckle, radial ring with inner-range baseline, azimuthal annulus, filament). Post-correction diagnostics reduced GridRad speckle to **1.8%** mean (**9.1%** P95) from **6.1%** after the initial three-pass filter. Residual ring-shaped structure in GridRad−MYRORSS difference maps over Oklahoma, the Northeast, and the northern Plains reflects both per-day geometry and **~1.7×** mid-range era calibration offset; an inner-range radial baseline was added to target wide elevated plateaus. Stages 05–07 were rebuilt after each revision; full hazard rerun (08–14) is gated on post-Stage-06 diagnostic review.
...
**v2.2.1 production hazard pipeline (Stages 05–14) is complete** on the full convective-day archive (validated **2026-06-30**).
...
**Radar artifact diagnostic (2026-07-06).** `scripts/diagnostics/radar_artifact_diagnostic.py` on the four-pass corrected archive (9,797 days) quantified NEXRAD geometry artifacts. GridRad mean speckle fraction (isolated spikes &gt;2.5× local 3×3 median) was **1.8%** of active cells (**9.1%** at P95) vs **3.9%** for MYRORSS—down from **9.7%** / **50%** pre-debias and **6.1%** / **33%** after the three-pass filter. Range-binned mean annual maxima show GridRad **~1.7×** MYRORSS at 50–125 km from radar. A GridRad−MYRORSS difference map retains ring-shaped structure over Oklahoma, the Northeast, and the northern Plains, reflecting residual per-day geometry and era calibration bias; an inner-range radial baseline (≤75 km reference for bins ≥50 km) was added to the Stage 05 filter to target wide mid-range plateaus. Full debias refit requires Stage 06 SPC pairs on the rebuilt archive.
...
Long-run monitoring (operations):           Full v2.2.1 production run complete 2026-06-30;
                                            Stages 01–14 validated (--skip-ml);
...
Main figures are rendered to `docs/figures/pnas/` by `scripts/diagnostics/render_pnas_article_figures.py`. Supplementary Stage 14 maps are in `docs/figures/historical/` and `docs/figures/analysis/`.
...
| 13 | `fig13_analytical_vs_stochastic.png` | Analytical vs stochastic RP comparison (Stage 14) |
