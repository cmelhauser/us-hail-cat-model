...
**Status:** Complete draft — v2.2.2 production hazard results; Stage 05 range debias + speckle filter (2026-07-05)  
...
**Range-dependent debias and speckle filter (2026-07).** Stochastic return-period maps initially showed NEXRAD range rings and spokes, traced to GridRad-era isolated speckle and range-dependent over-estimation relative to SPC collocated pairs. We added `scripts/diagnostics/radar_artifact_diagnostic.py`, which bins **173,766** validation pairs by distance to the nearest CONUS WSR-88D site (~140 radars) and fits per-era multiplicative debias factors (normalized at **125 km**, clipped **[0.45, 1.15]**). Stage 05 applies `range_debias.npz` when present. GridRad days additionally pass a **3×3 median speckle test** (zero cells &gt; **2.5×** local median among active pixels). On the full **9,797-day** corrected archive, GridRad mean speckle fraction fell from **9.7%** to **6.1%** and Stage 05 mean pixels filtered rose from **5.8%** to **17.2%**. Residual broad GridRad−MYRORSS mean-annual-max offsets (~2× at 50–125 km) reflect era sampling as well as artifacts; downstream Stages **06–14** were rerun after the debias rebuild.
...
| 9 | Stochastic RP radar rings/spokes | GridRad speckle 9.7%; SPC/MESH ~0.81 at 112 km; visible NEXRAD geometry in RP maps | `radar_artifact_diagnostic.py`; `range_debias.npz`; GridRad speckle filter in Stage 05 | Speckle 9.7%→6.1%; MRMS far-range annual max reduced; Stages 06–14 rerun | Residual GridRad−MYRORSS climatology offset; clustered speckle may remain |
