...
Catastrophe models are usually built by specialized teams over long development cycles. This work asks whether frontier AI systems can materially change that process. We describe the AI-assisted construction of a reproducible, radar-first US hail catastrophe hazard model using public data, a staged Python pipeline, and extensive documentation, tests, and validation artifacts. The model combines MYRORSS, GridRad, MRMS, ERA5, and SPC data to estimate hail occurrence, return periods, and stochastic event behavior. Beyond the hail results, the study documents a new mode of scientific software development: human-directed AI agents that research, implement, audit, and harden a full computational pipeline.
...
This paper presents a concrete case study: the construction of a US hail catastrophe hazard model using AI-assisted development. The work has two linked contributions. First, it describes a radar-first hail model that converts public atmospheric data into gridded hazard, return-period maps, and stochastic event catalogs. Second, it documents the process by which AI agents were used to accelerate literature review, pipeline implementation, debugging, testing, documentation, and run monitoring.

The title phrase "The Age of AI" is used deliberately. The point is not that AI replaces scientific judgment. Rather, the study examines a practical collaboration pattern: a human model owner sets goals, evaluates outputs, makes methodological decisions, and directs priorities, while AI systems perform high-volume implementation, synthesis, audit, and operational monitoring tasks. The resulting artifact is not merely a manuscript or a prototype, but a runnable repository with explicit assumptions, tests, validation commands, and documentation.
...
The AI-process analysis is also limited. This is a case study rather than a randomized comparison of human-only and AI-assisted development. The exact contribution of each AI system is difficult to quantify because the workflow was interactive and iterative. The article therefore frames AI as an enabling workflow component, not as an independently validated replacement for expert model development.
...
Large language models were used in the development of the codebase, documentation, monitoring workflow, and manuscript draft. The systems used were Claude Opus v6 and ChatGPT 5.5 [verify exact model/version names before submission]. AI outputs were reviewed, edited, tested, and accepted under human direction. AI systems are not listed as authors.
...
Input datasets are public or publicly documented. Generated data products are reproducible from the pipeline but are not committed to the source repository because of size.
