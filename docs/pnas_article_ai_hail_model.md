...
**Status:** Complete draft — v2.2.2 production hazard results; four-pass GridRad artifact filter with inner-range radial ring pass (2026-07-06)  
...
**Range-dependent debias and GridRad artifact filter (2026-07).** Stochastic return-period maps initially showed NEXRAD range rings and spokes, traced to GridRad-era isolated speckle and range-dependent over-estimation relative to SPC collocated pairs. We added `scripts/diagnostics/radar_artifact_diagnostic.py`, which bins validation pairs by distance to the nearest CONUS WSR-88D site (~140 radars) and fits per-era multiplicative debias factors (normalized at **125 km**, clipped **[0.45, 1.15]**). Stage 05 applies `range_debias.npz` when present. GridRad days pass a **four-pass artifact filter**: isolated speckle (&gt; **2.5×** local 3×3 median), **radial range ring** (per-site 10 km bins vs inner-range ≤75 km baseline and neighbors; **1.12×** / **1.18×** thresholds), azimuthal annulus, and quiet-background filament. On the **9,797-day** archive, GridRad mean speckle fell from **9.7%** (pre-debias) to **6.1%** (three-pass) to **1.8%** (four-pass; P95 **9.1%**). Residual GridRad−MYRORSS mean-annual-max offsets (~**1.7×** at 50–125 km) and ring-shaped diff-map structure over Oklahoma, the Northeast, and the northern Plains reflect era sampling and mid-range calibration bias as well as per-day artifacts; an **inner-range radial baseline** (2026-07-06) targets wide mid-range plateaus missed by neighbor-only comparison. Stages **05–07** were rebuilt after each filter revision; full hazard Stages **08–14** pending artifact QA.
