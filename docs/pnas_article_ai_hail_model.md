...
**Status:** Complete draft — v2.2.1 production hazard results; Stage 05 range debias + speckle filter (2026-07-05)  
...
**Range-dependent debias and speckle filter (2026-07).** Stochastic return-period maps initially showed NEXRAD range rings and spokes, traced to GridRad-era isolated speckle and range-dependent over-estimation relative to SPC collocated pairs. We added `scripts/diagnostics/radar_artifact_diagnostic.py`, which bins **173,766** validation pairs by distance to the nearest CONUS WSR-88D site (~140 radars) and fits per-era multiplicative debias factors (normalized at **125 km**, clipped **[0.45, 1.15]**). Stage 05 applies `range_debias.npz` when present. GridRad days additionally pass a **3×3 median speckle test** (zero cells &gt; **2.5×** local median among active pixels). On the full **9,797-day** corrected archive, GridRad mean speckle fraction fell from **9.7%** to **6.1%** and Stage 05 mean pixels filtered rose from **5.8%** to **17.2%**. Residual broad GridRad−MYRORSS mean-annual-max offsets (~2× at 50–125 km) reflect era sampling as well as artifacts; downstream Stages **06–15** were rerun after the debias rebuild.
...
| 9 | Stochastic RP radar rings/spokes | GridRad speckle 9.7%; SPC/MESH ~0.81 at 112 km; visible NEXRAD geometry in RP maps | `radar_artifact_diagnostic.py`; `range_debias.npz`; GridRad speckle filter in Stage 05 | Speckle 9.7%→6.1%; MRMS far-range annual max reduced; Stages 06–15 rerun | Residual GridRad−MYRORSS climatology offset; clustered speckle may remain |
...
**Radar artifact diagnostic (2026-07-05).** `scripts/diagnostics/radar_artifact_diagnostic.py` on the full corrected archive quantified NEXRAD geometry artifacts motivating the Stage 05 debias rerun. GridRad mean speckle fraction (isolated spikes &gt;2.5× local 3×3 median) was **9.7%** of active cells (**50%** at P95) vs **3.9%** for MYRORSS. Range-binned SPC/MESH ratios show GridRad over-estimation increasing with range (median **~0.81** at 112 km). After range debias and GridRad speckle filtering, speckle fell to **6.1%** (**33%** P95) and Stage 05 mean pixels filtered increased from **5.8%** to **17.2%**. A GridRad−MYRORSS mean-annual-max difference map remains broadly positive in the eastern CONUS (~2× at 50–125 km from radar), reflecting era sampling as well as residual calibration error.
...
The full pipeline contains 14 hazard and figure stages (Stage 14 vulnerability was removed; loss modeling is future work):
