...
Diagnostic summaries of daily peak hail across the unified `mesh_0.05deg` archive (not a pipeline stage output). Regenerated by `scripts/diagnostics/summarize_mesh_daily_peaks.py`. **Gitignored** — retain externally or regenerate locally.
...
### `data/analysis/hail_day_climatology/`

Per-cell severe-hail-day climatology and MESH75 threshold sensitivity on the Stage 05 corrected archive. Regenerated by `scripts/diagnostics/hail_day_climatology.py`. Benchmarks against Cintineo et al. (2012) and Murillo et al. (2021). **Gitignored** — retain externally or regenerate locally.

| File | Description |
|---|---|
| `threshold_benchmark_summary.csv` | Grid-wide and Great Plains hail-day stats at six literature thresholds |
| `national_annual_hail_days.csv` | CONUS any-cell hail-day counts by year and threshold |
| `monthly_national_hail_days.csv` | Monthly any-cell counts by threshold (seasonality diagnostic) |
| `hail_days_per_year_<threshold>.tif` | Per-cell mean annual hail days (GeoTIFF, one per threshold) |
| `map_hail_days_per_year_*.png` | Quick-look maps for key thresholds |
| `seasonal_national_hail_days_by_threshold.png` | National seasonal cycle by threshold |
| `README.md` | Literature benchmarks and latest summary table |
...
| `n_active_cells` | number of cells ≥ `EVENT_ACTIVE_THRESH_MM` (29.0 mm in v2.2.2) |
